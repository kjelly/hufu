...
Set the model with --model <name> (highest priority), in team.yaml, or in hufu.yaml.`,
	...
	rootCmd.Flags().StringArrayVar(&opts.varFlags, "var", nil, "Set a prompt/config template variable (key=value); this does not provide typed execution semantics")
	...
	rootCmd.Flags().StringArrayVar(&opts.inputFlags, "input", nil, "Set a typed run input (name=json-or-typed-scalar); repeatable")
	rootCmd.Flags().StringArrayVar(&opts.inputFiles, "input-file", nil, "Read typed run inputs from a JSON or YAML mapping; repeatable")
	...
	rootCmd.Flags().StringSliceVar(&opts.workerModelOverrides, "worker-model", nil, "Override the execution target of specific worker(s) as agent=target; repeatable or comma-separated")
	...
	rootCmd.Flags().StringVar(&opts.sidecarModelOverride, "sidecar-model", "", "Override sidecar LLM target used for skill matching")
	rootCmd.Flags().StringVar(&opts.guardModelOverride, "guard-model", "", "Override guard LLM target used for output review")
	...
	rootCmd.Flags().StringVar(&opts.planReviewerModelOverride, "plan-reviewer-model", "", "Override plan reviewer LLM target used for plan review")
	...
	rootCmd.Flags().DurationVar(&opts.gracefulWrapUpTimeout, "graceful-wrap-up-timeout", 0, "Maximum time to let active work finish after the first Ctrl+C (default: 15m or --timeout + 1m, whichever is longer)")
	...
	rootCmd.Flags().IntVar(&opts.compactionMaxHistoryMessages, "compaction-max-history-messages", 0, "Override compaction max history messages; zero keeps the safety default")
	rootCmd.Flags().IntVar(&opts.compactionRetainHistoryMessages, "compaction-retain-history-messages", 0, "Override compaction retained history messages; zero keeps the safety default")
	rootCmd.Flags().IntVar(&opts.compactionVerifiedHistoryTargetTokens, "compaction-verified-history-target-tokens", 0, "Override verified history target tokens; zero keeps the safety default")
	rootCmd.Flags().IntVar(&opts.compactionToolOutputMaxBytes, "compaction-tool-output-max-bytes", 0, "Override normalized tool output byte cap; zero keeps the safety default")
	rootCmd.Flags().IntVar(&opts.compactionToolOutputMaxRunes, "compaction-tool-output-max-runes", 0, "Override normalized tool output rune cap; zero keeps the safety default")
	rootCmd.Flags().IntVar(&opts.compactionToolOutputMaxTokens, "compaction-tool-output-max-tokens", 0, "Override normalized tool output token cap; zero keeps the safety default")
	rootCmd.Flags().IntVar(&opts.compactionDiagnosticMaxLines, "compaction-diagnostic-max-lines", 0, "Override preserved diagnostic line cap; zero keeps the safety default")
	rootCmd.Flags().IntVar(&opts.compactionDiagnosticMaxTokens, "compaction-diagnostic-max-tokens", 0, "Override preserved diagnostic token cap; zero keeps the safety default")
	...
	rootCmd.PersistentFlags().StringVar(&opts.executionProfile, "execution-profile", "", "Set execution profile: default, unattended, strict-verification, fresh-session, fresh-verification")
	...
	rootCmd.Flags().StringVar(&opts.themeMode, "theme", "", "Display theme: auto, light, dark, or mono (default: config or auto)")
	rootCmd.Flags().StringVar(&opts.displayPreset, "display-preset", "", "Display preset: default or epaper (default: config or default)")
