...
| `hufu context promotion <analyze\|list\|show\|edit\|approve\|reject\|apply>` | Review-gated promotion of confirmed persistent context into a team skill or coordinator/agent policy. `analyze`, explicit `approve`, and `apply` are separate; proposals and source snapshots live in `context.sqlite`, while content-free lifecycle events use the hash-chained event store. |
