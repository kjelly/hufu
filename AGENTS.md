...
> This working copy is the `kjelly/hufu` fork. The Go module path is `github.com/kjelly/hufu`. Releases are published from this fork via `.goreleaser.yml` -> `release.github.owner = kjelly`.
...
| `hufu team create <team> [--wizard]` | Deterministically creates a team, or explicitly enters a TTY-only preview/validate/write wizard. The wizard defaults to `readonly`, never overwrites, never runs the team, and never enables active learning. |
...
| `hufu context learning` | Shows scoped recall, usage, objective outcome, and promotion lifecycle counters. Unknown/unavailable data is not rendered as zero. |
| `hufu context promotion <analyze\|list\|show\|review\|edit\|approve\|reject\|apply>` | Review-gated promotion of confirmed persistent context into a team skill or coordinator/agent policy. Interactive `review`, explicit `approve`, and `apply` remain separate; proposals and source snapshots live in `context.sqlite`, while content-free lifecycle events use the hash-chained event store. |
| `hufu context consolidate [--apply-proposal --source <ids> (--proposal-text <text> \| --draft)]` | Dry run lists deterministic consolidation clusters (and members with unresolved memory conflicts). `--apply-proposal` persists a candidate derived from the sources, with operator text or, with `--draft`, strict-JSON text from the team sidecar model; either way the sources must be current, supported, and conflict-free, and `hufu context consolidation approve` is still required. |
| `hufu context conflicts <scan\|list\|show\|dismiss>` | Offline memory-conflict review. `scan` pairs confirmed persistent memories in the same scope by token overlap and shared file-path evidence (plus Ollama embedding neighbors with `--vector`, which needs a local embedding model) and asks the team sidecar model for a strict-JSON verdict, at most `--max-pairs` calls per scan; judged pairs are cached. `list`/`show` are read-only and content-free by default. Resolve an open conflict with `hufu context supersede <old> --with <new>` or `dismiss --reason`; nothing is superseded or dismissed automatically. |
...
- **Acceptance** (`acceptance:` in team.yaml) is an objective whole-run gate run at `finish`. In interactive mode a non-zero exit appends a failure note to the result and emits an `error` notification. In **unattended mode** it drives a self-healing loop: up to 2 retries (the coordinator is told to fix the failures and call `finish` again, tracked by `selfHealingAttempts`); if still failing, it runs the explicitly configured **rollback** command and reports the outcome. With no `rollback:` command, Hufu fails closed without changing the project.
...
rollback: ""                # unattended: explicit command run after self-healing fails; empty means no rollback mutation
...
54. **Self-healing acceptance + rollback (unattended)** — On acceptance failure in unattended mode, `finishTool.Run` retries up to `selfHealingAttempts` (2), returning an error response that tells the coordinator to fix and re-`finish`. After exhaustion it calls `runRollback` only for an explicit team `rollback:` command; otherwise it fails closed without mutating the project. Interactive mode keeps the original non-blocking "append failure note" behavior.
