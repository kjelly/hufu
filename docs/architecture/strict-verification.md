...
>
> 與 roadmap 的分工：本文件是 strict-verification 場景的**機制設計與驗收案例**；Go 子系統的 schema、詞彙與實作狀態以 code、tests、accepted ADR 及 `docs/roadmap.md` 的 active links 為準。本文的目標 schema 不自動代表 parser 已支援。
>
> - 本文件：聚焦 hufu 的 **strict verification／compliance workflow** 能力。
> - Active roadmap：只保留目前仍需追蹤的方向；歷史 backlog 見 `docs/archive/roadmaps/`。
>
...
> 「對應工作卡」是歷史 backlog 對照；目前 schema／詞彙／實作狀態以 code、tests、accepted ADR 與 `docs/roadmap.md` 為準。
...
> 本 Part 不是獨立 backlog。它保留歷史 HF-PR 對照；目前實作狀態、工作量、依賴、驗證指令與指派以 code、tests、accepted ADR 與 `docs/roadmap.md` 為準。
