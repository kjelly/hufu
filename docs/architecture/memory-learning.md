...
> Supersedes: the HF-MEM3 migration plans in `archive/implementation-plans/`
> Superseded-By: —

此文件是 L3/L4 的 runtime contract 與 rollout 驗收基準；不是要求重新
建立 canonical memory store 的 implementation plan。

> 基準程式碼：`6ab9951`（目前工作樹；歷史基準 `c9cb0a8...` 僅供追溯）
>
> 文件性質：依目前程式碼整理的規範與剩餘 rollout 邊界，不重做已完成的 HF-MEM3 canonical memory cutover。
>
