...
> Supersedes: the 2026-09-03 proposed model-metadata plan
> Superseded-By: —

**Implementation status:** The catalog/profile/resolver boundary, provenance
resolution, Ollama native `/api/show` and `/api/ps` introspection, bounded
runtime cache, and effective-context admission are implemented. The remaining
work is maintenance and provider-contract evolution; this document must not be
read as a request to create a second model metadata subsystem.
...
> 2026-09-03 審查備註（歷史基線）：本規格對「現況」的描述已逐項比對實際程式碼
> （`internal/team`、`internal/agent`、`cmd/hufu`）並確認準確；§1、§3.1、
> §13、§14、§17、§18.1 已補上精確的 file:line 依據與少量現況澄清，其餘
> 內容未變動。原始建議實作順序仍保留在 §27 作為歷史 rollout 分解；第一個 PR
> 曾聚焦 §1 item 2（`IsEstimated`
...
## 27. Historical rollout decomposition

The rollout below records how the implementation was decomposed. It is not an
open backlog: phases 1–4 are implemented and covered by the current
`internal/modelprofile` and `internal/providerintrospection` packages. Phase 5
is compatibility cleanup, not permission to reintroduce static model tables.
