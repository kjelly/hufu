...
**Supersedes:** `hufu-debiasing-decision-runtime-spec.md`, `hufu-strategic-decision-runtime-spec.md`, and the untracked root-level `spec.md` draft (2940 lines) that this document replaces.
...
> 本規格一律以本檔名 `docs/hufu-decision-aware-runtime-spec.md` 被引用；
> 新增程式碼註解必須引用本檔名，不得再寫 `spec.md`。
...
| 10 | Phase 4（capability routing）、Phase 5（calibration）移出 V1 | Phase 4 為綠地開發且與現有 capability 語意衝突；Phase 5 的 outcome resolution 在 CLI 無觸發機制、樣本數不足以支撐 Brier。兩者移至 §49 並附進入條件 |
...
本規格引入兩個 V1 runtime 抽象（第三個 `CapabilityResolver` 已延後，見 §49）：
...
| capability 記錄／評分／routing | `agent.CapabilityConfig` 只有 `Required []string`（`internal/agent/agent.go:473`）；`AgentPool` 只有 `ResolveAgentName`。現有註解明訂「capabilities 是描述性的，永遠不獨立授予工具」（`internal/team/services.go` `ResolvedWorkerTools`） | **延後至 §49 Phase 4** |
...
「為何選這個 agent」與「驗證後的結果教了我們什麼」屬 Phase 4/5，V1 不承諾。
...
`RoutingPolicy` 已從本結構移除（Phase 4，§49）。
...
> `on-capability-invalidated` 移至 Phase 4（§49）。
...
11. 新程式碼註解引用本規格時，使用完整檔名
    `docs/hufu-decision-aware-runtime-spec.md §N`，不得寫 `spec.md`。
...
以下兩個 phase 從草稿移出。它們並非被否決，而是**目前缺少可實作的前提**。
未滿足進入條件前不得開始。

### 49.1 Phase 4 — Capability-aware routing

**延後理由**

1. 現況為綠地：`agent.CapabilityConfig` 只有 `Required []string`
   （`internal/agent/agent.go:473`），`AgentPool` 只有 `ResolveAgentName`。
2. 與既有語意衝突：`ResolvedWorkerTools` 的註解明訂
   「capabilities 是描述性的，永遠不獨立授予工具」。要用它做 routing 分數，
   必須先變更該語意並重新檢視工具授權路徑。
3. 沒有可信的能力證據來源：規格要求「不信任自述」，但 V1 沒有已驗證結果可用
   （那需要 Phase 5）。
...
```text
Phase 3 完成且穩定
存在至少一個可信的能力證據來源（已驗證的歷史結果，或明確的維護者宣告）
已決定 capability 與工具授權的語意關係，且該變更本身有測試
```
