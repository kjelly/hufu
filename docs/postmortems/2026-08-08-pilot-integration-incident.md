...
> Status: historical
> Authority: reference
> Verified-Commit: incident `2026-08-08`
> Supersedes: —
> Superseded-By: [2026-08-09 remediation](2026-08-09-pilot-integration-remediation.md)
...
目前已移除的 team-owned contract（`hufu-pilot-integration/team.yaml` 與 `poc-contract-reader.md`）要求 reader：

- 不讀 source
- 不呼叫工具
- 只做一次 `submit_result`

但當時的 coordinator prompt（`coordinator.md:71`–`coordinator.md:103`）仍要求 reader：
...
- `hufu-pilot-integration/team.yaml`：加入 initial contract binding、closed sequence、reader/state probe static contract。
- `hufu-pilot-integration/coordinator.md`：加入 dynamic argv 不使用 scalar pinning 的通用規則，並縮短 §3.3 task contract。
- `hufu-pilot-integration/poc-contract-reader.md`：改為 context-only acknowledgement contract。
