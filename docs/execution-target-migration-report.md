...
This report records the execution-target migration required by `spec.md`.
...
`local` and `ollama` canonicalize to `local`. Named LLM and agent backends
share one namespace. A new `backends.<name>` entry cannot coexist with either
legacy namespace under the same canonical key. `backends.codex` uses
