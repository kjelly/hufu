...
func rca20DigestProjection() TaskOccurrenceProjection {
	return TaskOccurrenceProjection{
		...
		MaxRetries:     2, OnFailureClasses: []TaskFailureClass{FailureVerify, FailureSemanticRejection}, SideEffect: SideEffectWorkspaceWrite, Escalate: true, AdversarialVerify: 1, Recovery: RecoveryReconcile, ReconcileTool: "probe",
...
func TestRCA20DigestBindsEveryImmutableProjectionFieldExceptPlanID(t *testing.T) {
	...
	cases := []struct {
		...
		{"OnFailureClasses", func(p *TaskOccurrenceProjection) { p.OnFailureClasses = []TaskFailureClass{FailureTimeout} }},
