...
func fakeCodexThreadStartStep(threadID, cwd, model, sandbox string, t *testing.T) fakeCodexStep {
	return fakeCodexStep{Result: rawJSON(t, map[string]any{"thread_id": threadID, "cwd": cwd, "model": model, "sandbox": sandbox})}
}
...
func TestCodexNewThreadPersistsBindingBeforeTurn(t *testing.T) {
	...
	t.Run("persist success allows the turn to proceed", func(t *testing.T) {
		...
		turnResult, err := codexRunTurn(context.Background(), server.Client, effective.ThreadID, "do the work", "")
		if err != nil {
			t.Fatal(err)
		}
		...
		if got := server.CallLog(t); !slices.Equal(got, []string{"thread/start", "turn/start", "thread/read"}) {
			t.Fatalf("call log = %v, want thread/start, turn/start, thread/read in that order", got)
		}
	})
	...
	t.Run("persist failure blocks the turn from ever starting", func(t *testing.T) {
		server := startFakeCodexServer(t, []fakeCodexStep{
			...
			{Result: rawJSON(t, map[string]any{"turn_id": "turn-2"})}, // must never be consumed
		})
		defer server.Client.Close()
...
// ProviderBinding.SessionID drives thread/resume(thread_id), never
...
func TestCodexResumeUsesDurableThreadID(t *testing.T) {
	const cwd = "/workspace/task-1"
	server := startFakeCodexServer(t, []fakeCodexStep{
		{Result: rawJSON(t, map[string]any{"thread_id": "existing-thread", "cwd": cwd, "model": "gpt-5-codex", "sandbox": "workspace-write"})},
	})
	defer server.Client.Close()
