...
func taskTransitionPayloadWithCoordinator(item *TodoItem, c *Coordinator) map[string]interface{} {
	...
	payload := map[string]interface{}{
		...
		"action_input_bindings":            item.ActionInputBindings,
		"run_input_snapshot_id":            item.RunInputSnapshotID,
		"run_input_snapshot_hash":          item.RunInputSnapshotHash,
		"materialized_action_payload_hash": item.MaterializedActionPayloadHash,
		"bound_inputs":                     item.BoundInputs,
