// Package fsutil holds filesystem durability primitives shared by packages
// that must not depend on internal/team or internal/workspace: directory
// fsync, atomic replace/create, and non-blocking interprocess file locks.
package fsutil
...
// SyncDir performs an fsync operation on the specified directory to ensure
// directory entry changes are committed to disk.
...
func atomicTempPath(path string) string {
	...
	return fmt.Sprintf("%s.tmp.%s", path, hex.EncodeToString(randBytes))
}

// AtomicWriteFile safely writes data to path by writing to a temporary file,
// flushing to disk via Sync(), renaming atomically into place, and fsyncing
// the directory.
...
// exists, which closes the check/write race for create-only publication.
