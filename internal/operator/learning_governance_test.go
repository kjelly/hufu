...
func TestNormalizeSnapshotClonesGovernanceCounters(t *testing.T) {
	rejected, stale, edited, unknown := int64(1), int64(2), int64(3), int64(4)
	...
	rejected, stale, edited, unknown = 9, 9, 9, 9
	got := []*int64{normalized.Learning.RejectedPromotions, normalized.Learning.StalePromotions, normalized.Learning.AppliedEditedPromotions, normalized.Learning.AppliedEditUnknownPromotions}
