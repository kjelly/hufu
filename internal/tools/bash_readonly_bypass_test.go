...
func TestReadOnlyBashGrammarRejectsWriteFlags(t *testing.T) {
	denied := []struct {
		...
		{"go build with no -o writes binary to cwd", "go build ./cmd/hufu"},
		{"go build -o real path writes binary", "go build -o ./hufu ./cmd/hufu"},
		{"go build -o= real path writes binary", "go build -o=./hufu ./cmd/hufu"},
		{"go build -o non-null absolute path writes binary", "go build -o /tmp/hufu ./cmd/hufu"},
